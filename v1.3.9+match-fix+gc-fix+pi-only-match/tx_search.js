...
function validPsCompare(rdsPs, stationPs) {
    if (typeof stationPs !== 'string' || typeof rdsPs !== 'string') return false;
    ...
    // Strip leading and trailing underscores from rdsPs for core matching
    const strippedRdsPs = standardizedRdsPs.replace(/^_+|_+$/g, '');

    // Split stationPs into tokens (e.g., "__mdr___ _kultur_" -> ["__mdr___", "_kultur_"])
    // and pad short PS tokens to 8 characters so they're not bypassed
    const psTokens = stationPs
        .split(/\s+/)
        .filter(token => token.length > 0)
        .map(token => {
            const lower = token.toLowerCase();
            return lower.length < 8 ? lower.padEnd(8, '_') : lower;
        });

    for (let token of psTokens) {
        // Strip leading and trailing underscores from station token for core matching
        const strippedToken = token.replace(/^_+|_+$/g, '');

        // Exact core match (ignoring leading/trailing underscores)
        if (strippedToken === strippedRdsPs) {
            return true;
        }

        // Fuzzy matching: at least 3 character positions must match,
        // ignoring positions where rdsPs has an underscore
        let matchCount = 0;
...
async function fetchTx(freq, piCode, rdsPs) {
    ...
    const filteredLocations = Object.values(localDb.locations || {})
      .map(locData => ({
        ...
        stations: locData.stations.filter(station => {
          if (station.freq !== freq) return false;

          const piMatch = station.pi === piCode.toUpperCase() || station.pireg === piCode.toUpperCase();
          if (!piMatch) return false;

          // Instant match if PI is unique on this frequency
          if (isPiUniqueForFreq(freq, piCode)) return true;

          // Require at least 3 matching PS characters if PI is not unique
          return validPsCompare(rdsPs, station.ps);
        })
      }))
      .filter(locData => locData.stations.length > 0); // Keep only locations with matching stations
    ...
    if (filteredLocations.length > 1) {
        ...
        multiMatches = filteredLocations.filter(obj => obj !== match).slice(0, 5);
...
function isPiUniqueForFreq(freq, piCode) {
    const allStations = Object.values(localDb.locations || {}).flatMap(loc => loc.stations || []);
    const matchingFreqStations = allStations.filter(st => st.freq === freq);
    const piMatches = matchingFreqStations.filter(st => st.pi === piCode.toUpperCase());
    //console.log(`Freq: ${freq}, PI: ${piCode}, Matches: ${piMatches.length}, Unique: ${piMatches.length === 1}`);
    return piMatches.length === 1;
}
